...
# Import Phase 2 components (wrap in try/except so Phase 1 still works if Phase 2 missing)
try:
...
def chat(user_message: str, memory: memory_module.ConversationMemory, retriever, airline_filter=None) -> str:
    ...
    if booking_agent.is_booking_completion_message(user_message):
        ...
        if result.get("booking") and not result["booking"].get("error"):
            reply = (
                ...
                f"flight {result['booking'].get('flight_no', '')}, seat {result['booking'].get('seat', 'TBA')}, "
                f"status {result['booking'].get('status', 'confirmed')}."
            )
        else:
            reply = f"I couldn't find a booking for {result.get('pnr', '')}. Please check the PNR and try again."
        memory.add("assistant", reply)
        return reply

    # Pre-classifier short-circuit for explicit flight search / booking commands
    if booking_agent.is_flight_search_message(user_message) or any(kw in msg_lower for kw in FLIGHT_SEARCH_KEYWORDS):
    ...
    if AGENT_AVAILABLE:
        if any(kw in msg_lower for kw in FLIGHT_STATUS_KEYWORDS):
            ...
            return run_agent(user_message, memory, retriever)
    ...
    # PHASE 2 ROUTING — check agentic intents first
    if AGENT_AVAILABLE:
        # Check for disruption first (highest priority)
        is_disruption = (
            intent == "disruption" or
            any(kw in msg_lower for kw in DISRUPTION_KEYWORDS)
        )
        ...
        if is_disruption:
            ...
            if pnr:
                return handle_disruption(pnr, memory)
            elif flight_no:
                memory.add("user", user_message)
                response = (
                    f"I can see you're asking about flight {flight_no}. "
                    f"Could you share your PNR number so I can check your specific booking "
                    f"and find the best alternatives for you?"
                )
